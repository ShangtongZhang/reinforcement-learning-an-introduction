...
def accumulatingTrace(trace, activeTiles, lam, alpha):
    trace *= lam * DISCOUNT
    trace[activeTiles] += 1
    return trace
...
# @alpha: step size for all tiles
# @return: new trace for convenience
def replacingTrace(trace, activeTiles, lam, alpha):
...
# @alpha: step size for all tiles
# @return: new trace for convenience
def dutchTrace(trace, activeTiles, lam, alpha):
...
class Sarsa:
    ...
    def __init__(self, stepSize, lam, traceUpdate=accumulatingTrace, numOfTilings=8, maxSize=2048):
    ...
    for lamInd, lam in enumerate(lams):
        for alphaInd, alpha in enumerate(alphas):
            for run in range(runs):
                evaluator = Sarsa(alpha, lam, replacingTrace)
